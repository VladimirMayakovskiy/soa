...
def test_signup_success(user_service, db_session_user):
    payload = {
        "username": "alice",
        "email": "alice@example.com",
        "password": "strongpwd"
    }
    resp = user_service.post("/signup", json=payload)
    ...
    db_session_user.expire_all()
    user_in_db = db_session_user.query(User).filter(User.id == data["id"]).first()
...
def test_signup_conflict_email(user_service, db_session_user):
    bob = User(username="bob", email="bob@example.com", password=hash_password("123456"))
    db_session_user.add(bob)
...
def test_login_and_get_profile(user_service, db_session_user):
    raw_pwd = "secret123"
    ...
    db_session_user.add(user)
    db_session_user.commit()
    db_session_user.refresh(user)

    login_payload = {"username": "bob", "email": "bob@example.com", "password": raw_pwd}
    resp_login = user_service.post("/login", json=login_payload)
    ...
    resp_profile = user_service.get("/profile")
...
def test_update_profile_unauthorized_and_success(user_service, db_session_user):
    update_payload = {"first_name": "Charlie", "last_name": "Brown"}
    resp = user_service.put("/update", json=update_payload)
    ...
    resp_login = user_service.post("/login", json={"username": "dave", "email": "dave@example.com", "password": raw_pwd})
    ...
    resp_update = user_service.put("/update", json=new_data)
